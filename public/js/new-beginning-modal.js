/**
 * New Beginning — connect form modal (home page)
 */
(function () {
    'use strict';

    const modal = document.getElementById('new-beginning-modal');
    const form = document.getElementById('new-beginning-form');
    const successEl = document.getElementById('nb-form-success');

    if (!modal || !form) return;

    const openTriggers = document.querySelectorAll('.js-new-beginning-open');
    const closeTargets = modal.querySelectorAll('[data-nb-close]');
    let lastFocus = null;
    ...
    form.addEventListener('submit', function (e) {
        ...
        const decision = form.querySelector('[name="decision"]:checked');
        ...
        if (!decision) {
            const decisionFieldset = form.querySelector('[name="decision"]').closest('.nb-fieldset');
            markInvalid(decisionFieldset);
            valid = false;
        }

        if (valid) {
            KCFormSubmit.send('new-beginning', form, {
                onSuccess: function () {
                    form.reset();
